...
Steps:
  ...
  5. SET s:<name>_avail=true, s:<name>_method="pkg"
...
Steps:
  ...
  2. Parse assets for matching OS/arch
  3. Download to ~/tools/cache/<name>/
  4. Extract if archive (tar/zip)
  5. chmod +x, symlink to ~/tools/bin/<name>
  6. Verify: <tool> --version
...
Steps:
  1. git clone to ~/tools/cache/<name>/src/
  2. Check Makefile/setup.py/Cargo.toml/go.mod
  3. Install build deps if available
  4. Build + install
  5. Verify + clean artifacts
...
1. IF verify=y:
   ...
   IF post_install contains "setcap" → sudo setcap cap_net_raw+ep $(which <tool>)
