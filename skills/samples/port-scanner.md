...
```
When: IDS/IPS blocking standard SYN. Fragment pkts bypass simple rules.
...
```
When: frag+slow still detected. Hide source among decoys.
...
### P1: Pre-Flight
```
1. VERIFY scope: <target> in scope config?
   ✗ → LOG "REJECTED: out of scope", RET fail
...
2. RUN <cmd> timeout=(topN/100)*60s min, max 3600s
